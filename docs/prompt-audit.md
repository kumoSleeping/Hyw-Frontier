...
- `system.md`、`media_notice.md` 和最终回复示例统一要求复制 `display_url` 插图；原图 `url` 与网页 `source_url` 用于溯源，内部展示标识不作来源链接。
- 普通配图与截图裁剪共用 `media_images` / `media_attachment` 字段及历史恢复路径；整页截图仅供审阅，当前任务内可通过 `pageshot_id` 裁剪。
- `image_budget.md` 与截图工具说明统一为搜索、Reader 配图、整页截图和截图裁剪共享总预算；截图和裁剪各占一次额度。系统提示词已移除过时的“每轮最多10张”，选图建议统一为通常1–3张。
